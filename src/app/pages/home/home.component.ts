...
import { PeoplePageActions } from "../../core/state/people/actions";
...
ngOnInit(): void {
  this.actorACtrl.valueChanges.pipe(debounceTime(500), takeUntil(this._destroy)).subscribe(data=>{
    if(!data) return;
    this.store.dispatch(PeoplePageActions.searchCharacterByName({characterName: data, listName:'actorOneList'}));
  ...
  this.actorBCtrl.valueChanges.pipe(debounceTime(500), takeUntil(this._destroy)).subscribe(data=>{
    if(!data) return;
    this.store.dispatch(PeoplePageActions.searchCharacterByName({characterName: data, listName:'actorTwoList'}));
...
setPerson(character:IPeople,listName:string){
  this.store.dispatch(PeoplePageActions.loadCharacterDetails({character, listName}))
}
}
